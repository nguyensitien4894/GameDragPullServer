...
(function () {
    ...
    GlobalHeader.SetJackpot = (gameid, roomid) => {
        tnv = prompt("Vui lòng nhập tnv cần sét hũ ", "");
        var inputData = { Gameid: gameid, Roomid: roomid, DisplayName: tnv };

        $.ajax({
            type: "POST",
            url: GlobalHeader.Config.Root_Url + "Game/SetJackpot",
            data: JSON.stringify(inputData),
            contentType: "application/json; charset=utf-8",
            dataType: 'json',
	...
	GlobalHeader.getHistoryThankfulList = function (currPage) {
        var nickname = $('#nickname').val().trim();
        var gamemode = $('#gameMode').val().trim();
        var fromDate = $('#from').val().trim();
        var toDate = $('#to').val().trim();
        //var type = $('#Type').val().trim();
        //var status = $('#status').val().trim();
        //var serviceId = $('#serviceId').val().trim();
        var param = '?nickname=' + nickname + '&fromDate=' + fromDate
            + '&toDate=' + toDate + '&currentPage=' + currPage + '&gamemode=' + gamemode;
        //+ '&code=' + code + '&status=' + status + '&serviceId=' + serviceId  + '&type=' + type
        $.ajax({
            type: "GET",
            url: GlobalHeader.Config.Root_Url + "CustomerSupport/getHistoryThankfulAccountList" + param,
            xhrFields: { withCredentials: true },
            crossDomain: true,
            success: function (html) {
                $('#hisCardExchange').html(html);
