...
namespace MsWebGame.Portal.Database.DAO
{
    public class UserPrivilegeDAO
    {
        ...
        /// <param name="Response"></param>
        public void UserPrivilegeGratitude(long UserID, int RankID, string Description,int ServiceID, out long RefundAmount, out long UserRedemptionID, out long RemainBalance,out int Response)
        {
            DBHelper db = null;
            try
            {
                ...
                param[7] = new SqlParameter("@_RemainBalance", SqlDbType.BigInt);
                ...
                if (Response == 1)
                {
                    ...
                    RemainBalance = Convert.ToInt64(param[7].Value);
                }
                else
                {
                    UserRedemptionID = 0;
        ...
        public List<DTO.UserRedemption> UserRedemptionHistoryList(long UserRedemptionID,long UserID, string UserName, int? RankID, int ServiceID, int CurrentPage, int RecordPerpage, out int TotalRecord)
        {
            DBHelper db = null;
            try
            {
