...
using System.Linq;
using MsWebGame.Portal.Helpers.OTPs.OtpAps;
...
namespace WebGame.Payment.Database.DAO
{
    public class SMSDAO
    {
        ...
        public void SPSmsOTPVerify(int ServiceID, long UserID, string Type, string Otp, string Msisdn, int Status, out int Response)
        {
            DBHelper db = null;
            try
            {
                ...
                db.ExecuteNonQuerySP("SP_SmsOTP_Verify", param.ToArray());
        ...
        public void SPSmsODPVerify(int ServiceID, long UserID, string Type, string Otp, string Msisdn, int Status, out int Response)
        {
            DBHelper db = null;
            try
            {
                ...
                param[6] = new SqlParameter("@_Status", SqlDbType.Int);
                param[6].Value = Status;
                db.ExecuteNonQuerySP("SP_SmsODP_Verify", param.ToArray());
                Response = ConvertUtil.ToInt(param[5].Value);
                return;
            }
            catch (Exception ex)
        ...
        public void OTPInit( long UserID, string Type, string Otp, string Msisdn,int ServiceID,out long Balance, out long CurrentBalance,out long Response,out int OtpFeePerTime)
        {
            DBHelper db = null;
            try
            {
        ...
        public void ODPInit(long UserID, string Type, string Otp, string Msisdn, int ServiceID, out long Balance, out long CurrentBalance, out long Response, out int OtpFeePerTime)
        {
            DBHelper db = null;
            try
            {
                ...
                param[8] = new SqlParameter("@_ServiceID", SqlDbType.Int);
                param[8].Value = ServiceID;
                db.ExecuteNonQuerySP("SP_AZTechODP_Insert", param.ToArray());
                Response = Convert.ToInt32(param[6].Value);
                if (Response > 0)
                {
                    Balance = Convert.ToInt64(param[4].Value);
        ...
        public void UpdateStatusODP(long RequestId, int Active, int Status, string RequestTime, out int Response, out int OtpFeePerTime, out long Balance, out long CurrentBalance)
        {
            DBHelper db = null;
            try
            {
                ...
                param[7] = new SqlParameter("@_Response", SqlDbType.Int);
                param[7].Direction = ParameterDirection.Output;
                db.ExecuteNonQuerySP("SP_AZTechODP_UpdateStatus", param.ToArray());
                Response = Convert.ToInt32(param[7].Value);
                if (Response == 1)
                {
                    Balance = Convert.ToInt64(param[4].Value);
        ...
        public void ValidOtpFistTele(long AccountID, string Phone, string Otp, int ServiceID, out int Response, out long OtpID, out string msg)
        {

            Otp = Otp.Trim();
            msg = string.Empty;
            if (Otp.Length != 6 && Otp.Length != 7 && Otp.Length != 5)
            {
                Response = -1;
                OtpID = 0;
                return;
            ...
            try
            {
                ...
                db.ExecuteNonQuerySP("SP_OTP_CheckValid_TeleFist", param.ToArray());
