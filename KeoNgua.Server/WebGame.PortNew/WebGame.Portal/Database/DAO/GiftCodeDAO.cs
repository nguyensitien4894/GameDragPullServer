...
namespace MsWebGame.Portal.Database.DAO
{
    public class GiftCodeDAO
    {
        ...
        public int ReceiveGiftCode(long UserID, string GiftCode, string deviceId, int ServiceID, out long balance,out long giftCodeId,out long GiftCodeAmount)
        {
            balance = 0;
            DBHelper db = null;
            try
            {
                ...
                int response = Convert.ToInt32(param[5].Value);
                if (response == 1)
                {
                    balance = ConvertUtil.ToLong(param[3].Value);
                    giftCodeId= ConvertUtil.ToLong(param[4].Value);
                    GiftCodeAmount = ConvertUtil.ToLong(param[6].Value); ;
                }else
                {
                    balance = 0;
                    giftCodeId = 0;
        ...
        public List<GameGiftCode> GetListGameGiftCode(long giftCodeID, long campaignID, string giftCode, bool? status,int ServiceID)
        {
            DBHelper db = null;
            try
            {
