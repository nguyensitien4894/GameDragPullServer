...
namespace TraditionGame.Utilities
{
    ...
    public static class ConvertUtil
    {
        ...
        {
            return String.IsNullOrEmpty(s) ? null : s.StartsWith("0") ? "855" + s.Substring(1) : s;
        }

        public static string PhoneDisplayFormat(this string s)
        {
            return String.IsNullOrEmpty(s) ? null : s.StartsWith("855") ? "0" + s.Substring(2) : s;
        }
        ...
        public static long AccountIdBot(long accountId)
        {
            try
            {
                if (accountId > Config.MAX_BOTID) return accountId;
