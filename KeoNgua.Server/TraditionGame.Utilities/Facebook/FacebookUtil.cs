...
namespace TraditionGame.Utilities.Facebook
{
    public class FacebookUtil
    {
        ...
        public static FbUserInfoScore GetFbUserInfoScore(string accessToken)
        {
            try
            {
                var resultApp = GetDataUrl.RequestResponse(GraphApiUri + "app?access_token=" + accessToken);
                ...
                if (appInfo.id > 0)
                {
                    var resultGetUser = GetDataUrl.RequestResponse(GraphApiUri + "me?fields=id,email,first_name,last_name&access_token=" + accessToken);
                    dynamic userInfo = Newtonsoft.Json.JsonConvert.DeserializeObject(resultGetUser);
                    if (userInfo.id > 0)
                    {
                        return new FbUserInfoScore()
                        {
                            UserId = userInfo.id,
                            Email = userInfo.email,
